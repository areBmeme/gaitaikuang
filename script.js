...
function initAboutPageInteraction() {
    ...
    // 四大行业痛点导航功能
    const painNavBtns = document.querySelectorAll('.pain-nav-btn');
    const solutionPanels = document.querySelectorAll('.solution-panel');
    
    painNavBtns.forEach(btn => {
        btn.addEventListener('click', function() {
            const painId = this.getAttribute('data-pain');
            
            // 移除所有按钮的active状态
            painNavBtns.forEach(b => b.classList.remove('active'));
            // 添加当前按钮的active状态
            ...
            // 显示对应的解决方案面板
            const targetPanel = document.querySelector(`[data-solution="${painId}"]`);
            if (targetPanel) {
    ...
    // 为表格单元格添加data-label属性（移动端响应式）
    const tableCells = document.querySelectorAll('.table-cell:not(.header)');
    const tableHeaders = document.querySelectorAll('.table-cell.header');
    
    if (tableHeaders.length > 0) {
        tableCells.forEach((cell, index) => {
            const rowIndex = Math.floor(index / tableHeaders.length);
            const colIndex = index % tableHeaders.length;
            const headerText = tableHeaders[colIndex]?.textContent || '';
...
function initValueTabs() {
    ...
    if (valueTabBtns.length === 0 || valuePanels.length === 0) {
        return; // 如果元素不存在，直接返回
    }
    
    // 确保第一个面板是显示的
    valuePanels.forEach((panel, index) => {
        if (index === 0) {
    ...
    valueTabBtns.forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            // 显示对应的价值面板
            const targetPanel = document.querySelector(`.value-panel[data-value="${valueType}"]`);
            if (targetPanel) {
                targetPanel.classList.add('active');
